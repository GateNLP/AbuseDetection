...
from gatelfpytorchjson import LayerLSTM
import sys
import logging
...
class TextClassBiLSTMcnnSingle(CustomModule):
    ...
    def __init__(self, dataset, config={}):
        ...
        layer_lstm = LayerLSTM(self.layer_emb.emb_dims, config=config)
        layer_cnns = LayerCNN(layer_lstm.out_dims, config=config)
        layer_lin = torch.nn.Linear(layer_cnns.dim_outputs, self.n_classes)
        logsoftmax = torch.nn.LogSoftmax(dim=1)

        self.layers = torch.nn.Sequential()
        self.layers.add_module("layer_lstm", layer_lstm)
    ...
    def forward(self, batch):
        ...
        if self.on_cuda():
            batch.cuda()
        embed = self.layer_emb(batch)
        out = self.layers(embed)
